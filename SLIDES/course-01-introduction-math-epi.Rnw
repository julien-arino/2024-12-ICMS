...
<<KMK_SI_plane,eval=TRUE,echo=FALSE>>=
...
\vfill
\begin{center}
\includegraphics[width=\textwidth]{\Sexpr{fig_chunk('KMK_SI_plane', 'pdf')}}
\end{center}
\end{frame}
...
\end{lstlisting}
\begin{center}
\includegraphics[width=\textwidth]{\Sexpr{fig_chunk('KMK_R0eq1dot5', 'pdf')}}
\end{center}
\end{frame}
...
\begin{frame}{Attack rate (in \%)}
  \begin{center}
    \includegraphics[width=\textwidth]{\Sexpr{fig_chunk('KMK_attack_rate', 'pdf')}}
  \end{center}
\end{frame}
